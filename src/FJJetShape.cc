...
Double32_t FJJetShapeGRNum::result(const fastjet::PseudoJet &jet) const {
...
Double32_t FJJetShapeGRDen::result(const fastjet::PseudoJet &jet) const {
  if (!jet.has_constituents())
    return 0; //AliFatal("Angular structure can only be applied on jets for which the constituents are known.");
...
Double32_t FJJetShapeAngularity::result(const fastjet::PseudoJet &jet) const {
  if (!jet.has_constituents())
    return 0; 
  Double_t den=0.;
  Double_t num = 0.;
...
Double32_t FJJetShapepTD::result(const fastjet::PseudoJet &jet) const {
  if (!jet.has_constituents())
    return 0; 
  Double_t den=0;
  Double_t num = 0.;
...
Double32_t FJJetShapeCircularity::result(const fastjet::PseudoJet &jet) const {
  if (!jet.has_constituents())
    return 0;
  Double_t mxx    = 0.;
  Double_t myy    = 0.;
...
Double32_t FJJetShapeSigma2::result(const fastjet::PseudoJet &jet) const {
  if (!jet.has_constituents())
    return 0;
  Double_t mxx    = 0.;
  Double_t myy    = 0.;
